...
class Trainer
{
    constructor(id,name,gender,money,pokemonTeam,totalPokemon,expShare,items,isDefeated,img,faintedQuantity)
    {
        this.id=id;//Int. Trainer no.
...
new Item("ether",20,20,"PUT AN IMAGE OBJECT HERE IF TIME ALLOWS!!");
new Item("ether",20,20,"PUT AN IMAGE OBJECT HERE IF TIME ALLOWS!!");
new Item("ether",20,20,"PUT AN IMAGE OBJECT HERE IF TIME ALLOWS!!");

    //name,quantity,value,img
items[0]=new Item("ether",20,20,"PUT AN IMAGE OBJECT HERE IF TIME ALLOWS!!");
items[1]=new Item("potion",20,20,"PUT AN IMAGE OBJECT HERE IF TIME ALLOWS!!");
items[2]=new Item("revive",20,20,"PUT AN IMAGE OBJECT HERE IF TIME ALLOWS!!");
items[3]=new Item("maxrevive",20,20,"PUT AN IMAGE OBJECT HERE IF TIME ALLOWS!!");
items[4]=new Item("pokeball",20,20,"PUT AN IMAGE OBJECT HERE IF TIME ALLOWS!!");
items[5]=new Item("maxether",20,20,"PUT AN IMAGE OBJECT HERE IF TIME ALLOWS!!");
...
//id, name,gender,money,pokemonTeam,totalPokemon,expShare,items,isDefeated,img,faintedQuantity
var player = new Trainer(1,"Ash","boy",500,team,totalPokemon,true,items,false);
...
function addPokemon(caughtPokemon)
{
    ...
    variables="?trainerno="+caughtPokemon
            +"&dexnum="+caughtPokemon.dexNumber
            +"&level="+caughtPokemon.level
            +"&hp="+caughtPokemon.maxHp
            +"&attack="+caughtPokemon.attack
            +"&defense="+caughtPokemon.defense
            +"&specialattack="+caughtPokemon.specialAttack
            +"&specialdefense="+caughtPokemon.specialDefense
            +"&speed="+caughtPokemon.specialAttack 
            +"&exp="+caughtPokemon.exp
            ;
            //Use JSON array to do this.
            ...
            //console.log("variables index #"+i+":"+variables);
            //Send the JS data into the file!
            //xhttp.open("GET",url,true);
    
            //xhttp.send(variables[i]);
            //xhttp.open("GET",url,true);
            //xhttp.send();
        ...
        //var url="test.php";
        //This PHP script sets the latest data to the database.
        var url="http://localhost/set_battle_data.php";
        //EACH REQUEST ELEMENT IS A POKEMON
        ...
        xhttp.open("GET",url+variables,true);
...
//Thanks to this. I can pass on the movename into this function.
            ...
            case "pokeball":
                {
                    ...
                    var caught=catchPokemon();
                    
                    if(caught==true)
                    {
                        catchMsg=player.name+" caught "+opponent.pokemonTeam[opponentCurrentPokemon].name+"!";
                    }//End if
                    else
                    {
                        catchMsg=player.name+" failed to catch "+opponent.pokemonTeam[opponentCurrentPokemon].name+"!";
                    }//End else
                    load();
                    display(catchMsg);
                    break;
...
function catchPokemon()
{
    ...
    console.log("catch chance is:"+catchChance);
    if(catchChance>=opponent.pokemonTeam[opponentCurrentPokemon].level)
    {
        ...
        //Add the Pokemon as is, then kill it.
        ...
        if(opponent.isWild==false)
        {
            //Just announce it to be dead and be done with it.
            opponent.pokemonTeam[opponentCurrentPokemon].hp=0;
            opponent.pokemonTeam[opponentCurrentPokemon].isAlive=false;
        }//End if
        load();
        return true;
    }//End if
    else //If it fails, then return false.
    {return false;}
...
{   
    ...
    attackTotal+=player.pokemonTeam[playerCurrentPokemon].attack+14;
    var health=document.getElementById("playerhpbar");
...
function init()
{
    //This code here sets the current values of the POkemon. So put this in the end.
    

    //This PHP script gets the battle data from the database and puts it into the Pokemon objects.
    //This returns the data with json encode
    var url="http://localhost/get_battle_data.php";
    //EACH REQUEST ELEMENT IS A POKEMON
    var xhttp=new XMLHttpRequest();
    var receivedData;
        ...
        xhttp.onreadystatechange=function()
        {
            ...
            if(this.readyState==4&&this.status==200)
            {
                ...
                //In here, just loop it and assign here.
                for(i=0;i<numPlayerPokemon;i++)
                {   console.log("iteration # "+i+"!!!!!!");
                    console.log("bro wHAT "+receivedData[i].Level);
                    //Need to parse the int to successfully extract the data.
                    console.log("received levels "+receivedData[i].DexNum);
                    //ASSIGNING VARIABLES
                    
                    player.id=parseInt(receivedData.TrainerNo);
                    player.pokemonTeam[i].dexNumber=parseInt(receivedData[i].DexNum);
                    player.pokemonTeam[i].level=parseInt(receivedData[i].Level);
                    player.pokemonTeam[i].maxHp=parseInt(receivedData[i].CurHP);
                    player.pokemonTeam[i].attack=parseInt(receivedData[i].CurAtk);
                    player.pokemonTeam[i].defense=parseInt(receivedData[i].CurDef);
                    player.pokemonTeam[i].specialAttack=parseInt(receivedData[i].CurSpAtk);
                    player.pokemonTeam[i].specialDefense=parseInt(receivedData[i].CurSpDef);
                    player.pokemonTeam[i].speed=parseInt(receivedData[i].CurSpeed);
                    player.pokemonTeam[i].exp=parseInt(receivedData[i].EXP);
                    console.log(typeof(parseInt(receivedData[i].DexNum)));
                    console.log(typeof(player.pokemonTeam[i].dexNumber));
                    console.log("DEX NUMBER RIGHT NOW "+player.pokemonTeam[i].dexNumber);
                    console.log("ID IS  "+ player.id);
                }//End for
    ...
    //console.log("MY RECEIVED DATA IS: "+receivedData);
...
*/
    ...
    */
    ...
    {
        console.log("iter "+ i);
        console.log("MY TEAM!!! dex IS "+player.pokemonTeam[i].dexNumber);
    }
    load();
}//End init
...
-----------------PUT DATABASE STUFF HERE (UPDATE)-----------------    
    */
    ...
    //Passing the variables into db.
    //This code here sets the current values of the POkemon. So put this in the end.
    player.pokemonTeam[0].dexNumber=53;
    player.pokemonTeam[1].dexNumber=50;
    player.pokemonTeam[2].dexNumber=20;
    player.pokemonTeam[3].dexNumber=21;

    var variables=new Array();
    //This loop is for setting the variables for the db
        for(i=0;i<numPlayerPokemon;i++)
        {
            console.log("Iteration #"+i);
            variables[i]="?trainerno="+player.id
            +"&dexnum="+player.pokemonTeam[i].dexNumber
            +"&level="+player.pokemonTeam[i].level
            +"&hp="+player.pokemonTeam[i].maxHp
            +"&attack="+player.pokemonTeam[i].attack
            +"&defense="+player.pokemonTeam[i].defense
            +"&specialattack="+player.pokemonTeam[i].specialAttack
            +"&specialdefense="+player.pokemonTeam[i].specialDefense
            +"&speed="+player.pokemonTeam[i].specialAttack 
            +"&exp="+player.pokemonTeam[i].exp
            ;
            //Use JSON array to do this.
            ...
            //Parse into JSON
            //console.log(JSON.parse(this.responseText));
            console.log("variables index #"+i+":"+variables[i]);
            //Send the JS data into the file!
            //xhttp.open("GET",url,true);
        ...
        xhttp[i].open("GET",url+variables[i],true);
        xhttp[i].send(variables[i]);
        }//End for
