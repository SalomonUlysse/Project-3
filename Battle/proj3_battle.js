...
//Random background
var bgSet = Math.floor(Math.random() * 6) + 1;
var bgImg;
function setBackground() {
    bgImg = new Image();
    var i = bgSet.toString() + ".png";
    document.body.style.backgroundImage = "url(" + i + ")";
}

// Function calls at beginning of program.
//Check for height
var pHeight, oHeight, pWidth, oWidth;
// Player's dx, dy, dWidth, dHeight;
var dx, dy, dWidth, dHeight;
// Same for opponent
var odx, ody, odWidth, odHeight;
// Canvases variables
var canvas = document.getElementById('player');
var canvas2 = document.getElementById('opp');
var ctx = canvas.getContext('2d');
var ctx2 = canvas.getContext('2d');

function origPos(pStr, oStr) {
    window.player = new Image();
    player.onload = function(){ setPlayer(ctx, "p", player, this.width, this.height); }
    player.src = '../img/1st Generation/Player/' + pStr + '.png';
    pHeight = player.height;
    pWidth = player.width;

    window.opp = new Image();
    opp.onload = function() { setPlayer(ctx2, "o", opp, this.width, this.height); }
    opp.src = '../img/1st Generation/Opponent/' + oStr + '.png';
    oHeight = opp.height;
    oWidth = opp.width;
}

function setPlayer(ctx, str, img, height, width){
  switch (bgSet) {
          case 1:
              dx = 0;
              dy = 800 - height*0.3;
          ...
          case 6:
              ...
              break;
          default:
              console.log("Which files are you accessing ?");
              break;
          }
        if (str == "p"){
          ...
          ctx.drawImage(img, odx, ody, odWidth, odHeight);
        }
}
...
for(i=0;i<numPlayerItems;i++)
{
    ...
    items[i]=new Item("ether",1,20,"../img/Items/Health Potion.png");
}//End for

//This loop initializes the opponent's items.
for(i=0;i<numOpponentItems;i++)
{
    opponentItems[i]=new Item("potion",1,40,"../img/Items/Health Potion.png");
}//End for
